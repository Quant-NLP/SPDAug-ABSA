...
def mask_embedding(padded, mask_index):
...
def AE(df):
    ...
    for i in range(len(df)): 
        ...
        for pertubed_index in range(sep_index+1, MAX_LEN):
            ...
            if padded[0][pertubed_index] != 0 and padded[0][pertubed_index] != sep_id:
                ...
                cur_embedding = mask_embedding(padded, mask_index)
    ...
    for i in range(len(df)):
        tokenized = tokenizer.encode(df['text'].iloc[i])
        tokenized = torch.Tensor(tokenized).unsqueeze(0).to(torch.int64).to(device)
        augment_tokenized = tokenizer.encode(df['text'].iloc[i])

        for j in range(len(df['perturbed_mask_index'].iloc[i])):
            mask_tokenized = tokenizer.encode(df['text'].iloc[i])
            perturbed_mask_index = df['perturbed_mask_index'].iloc[i][j] + 1
    ...
    for i in range(len(df)):
        df['augment_text'].iloc[i] = tokenizer.decode(df['augment_token_id'].iloc[i][1:-1])
